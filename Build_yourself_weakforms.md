## Futureye Example: How to use WeakFormBuilder ##

Although we have defined many weak forms of different types of PDEs, sometimes we may need to define ourself's weak forms.
Here, we provide a new method (since version2.1) to define weak forms directly in you current code file. It is easier than the old method (extends AbstractXXXWeakForm or implements interface WeakForm). Let's see the example below.
...
public class UseWeakFormBuilder {
	...
	public static void triangleMesh() {
        ...
        Function fx = FX.fx;
        Function fy = FX.fy;
		wfb.addParamters(fx.M(fx).A(fy.M(fy)).M(-2.0).A(36.0),"f");
